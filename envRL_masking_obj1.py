...
ship = Ship(targeted_fish='Cod', targeted_quantity=800, weight=1000)
...
class Routing(Env):
    def __init__(self, Graph: Graph, Ship: Ship, start):
        ...
        self.observation_space = Dict({'route': Discrete(7), 'pathDistance': Box(low=0, high=30000, shape=(1,), dtype=float)})
        self.current_place = start
        locations.index(start)
        self.state = {'route': locations.index(start), 'pathDistance': Box(low=0, high=30000, shape=(1,), dtype=float)}
    ...
    def step(self, action):
        ...
        if location in self.successorsLst:
            ...
            self.state = {'route': action, 'pathDistance': distance}
            reward = -distance
        else:
            self.state = {'route': locations.index(self.previous_place),
                          'pathDistance': -np.inf}
            reward = -np.inf
        info = {}
    ...
    def reset(self, seed=None, options=None):
        ...
        self.previous_place = None
        self.route_length = 10
        self.state = {'route': locations.index(start), 'pathDistance': 0}
        info = self.get_info()
