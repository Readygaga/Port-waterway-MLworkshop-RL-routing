...
graph1 = Graph(G)
loc1 = FishGround(name='FG1', location=Position(30, 15), fishstock={'Cod': {'quantity': 500, 'timewindow': (1, 2)}})
...
ship = Ship(targeted_fish='Cod', targeted_quantity=3000, weight=1000)
...
class Routing(Env):
    ...
    def step(self, action):
        ...
        # edgeattr['starting time'] = self.time
        # edgeattr['finishing time'] = self.time + edgeattr['duration']
        # edgeattr['fuel consumption'] = passageFuel_Consumption(ship,
        #                                                        edgeattr['distance'],
        #                                                        edgeattr['speed'])
        #edgeattr['risk'] = passageRisk_trial(ship, speed=edgeattr['speed'], distance=edgeattr['distance'])
        ...
        # nodeattr['arrival time'] = self.time + edgeattr['duration']
        # nodeattr['departure time'] = nodeattr['arrival time'] + nodeattr['duration']
        # nodeattr['fuel consumption'] = locFuel_Consumption(ship, duration=nodeattr['duration'])
        #nodeattr['risk'] = locRisk_trial(ship, duration=nodeattr['duration'])
        nx.set_node_attributes(self.Footprint, {self.current_place: nodeattr})
        # self.time = nodeattr['departure time']
    ...
    def render(self):
        ...
        nx.draw_networkx_edge_labels(self.Footprint, pos_for_speed,
                                     edge_labels={(u, v): "Sp:{:.0f}".format(d['speed']) for u, v, d in
    ...
    def reset(self, seed=None, options=None):
        ...
        self.time = 0
        loc1.fishstock['Cod']['quantity'] = 500
