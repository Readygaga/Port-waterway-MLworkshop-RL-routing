...
class Routing(Env):
    ...
    def step(self, action):
        ...
        if location in self.successorsLst:
            ...
            self.state = {'route': locations.index(self.previous_place), 'Speed': action[1], 'Duration': action[2], 'pathDistance': -100000000}
            reward = -100000000
        info = {}
...
model = PPO('MlpPolicy', envR, verbose=1, tensorboard_log=log_path)
model.learn(total_timesteps=total_timesteps)
model.save(model_path)

model_path = os.path.join('Training', 'Model', f'{total_timesteps}stepModel')
model = PPO.load(model_path, env=envR)
#eva = evaluate_policy(model, envR, n_eval_episodes=10, render=False)
#print(eva)

obs, info = envR.reset()
#obs = envR.observation_space
print(obs)
